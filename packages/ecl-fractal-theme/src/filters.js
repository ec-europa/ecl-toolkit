...
module.exports = (theme, env, app) => {
  ...
  env.engine.addFilter('beautify', str => beautifyHTML(str, {
    // TODO: move to config
    indent_size: 4,
  ...
  env.engine.addFilter('resourceUrl', str => `/${app.web.get('assets.mount')}/components/${Path.relative(Path.resolve(app.components.get('path')), Path.resolve(str))}`);
  env.engine.addFilter('componentPath', str => Path.relative(process.cwd(), Path.join(app.components.get('path'), Path.relative(Path.resolve(app.components.get('path')), Path.resolve(str)))));
